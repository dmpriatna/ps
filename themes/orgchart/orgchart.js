...
(function ($) {
	...
	var nodeCount = 0;
	var uri = location.origin + '/' + location.pathname.split('/')[1];
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append('<div><img class="cover" src="' + uri + '/themes/orgchart/images/orgchart.minus.png"/></div>');
		}
		...
		if ($childNodesCount > 0) {		
			$nodeDiv.next().children('img.cover').click(function () {
				...
				if ($tr.hasClass('contracted')) {
					...
					$(this).attr('src', uri + '/themes/orgchart/images/orgchart.minus.png');
					...
					$(this).attr('src', uri + '/themes/orgchart/images/orgchart.plus.png');
		...
		if ($node.attr('class') != undefined) {
			...
			$.each(classList, function (index, item) {
				if (item == 'collapsed') {
					...
					$nodeRow.find('img.cover').attr('src', uri + '/themes/orgchart/images/orgchart.plus.png');
				} else {
